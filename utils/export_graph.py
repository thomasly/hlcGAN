...
def export_graph(model_name, XtoY=True):
    ...
    with graph.as_default():
        cycle_gan = CycleGAN(
            ngf=FLAGS.ngf, norm=FLAGS.norm, image_size=FLAGS.image_size
        )

        input_image = tf.placeholder(
            tf.float32, shape=[FLAGS.image_size, FLAGS.image_size, 3],
            name='input_image'
        )
        cycle_gan.model()
        if XtoY:
    ...
    with tf.Session(graph=graph) as sess:
        ...
        tf.train.write_graph(
            output_graph_def, 'pretrained', model_name, as_text=False
        )
