...
class Reader():
    def __init__(
        self, tfrecords_file, image_size=256,
        ...
        min_queue_examples: integer, minimum number of samples to retain in
                            the queue that provides batches of examples
    ...
    def _preprocess(self, image):
        ...
        image = helpers.convert2float(image)
    ...
    with tf.Graph().as_default():
        ...
        try:
            ...
            while not coord.should_stop():
                batch_images1, batch_images2 = sess.run(
                    [images_op1, images_op2])
                print('image 1 shape: {}'.format(batch_images1))
                print('image 2 shape: {}'.format(batch_images2))
                print('='*10)
                step += 1
